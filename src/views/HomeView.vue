...
async function handleWishSubmit(wish) {
  ...
  currentStep.value = 3;
  await new Promise((resolve) => setTimeout(resolve, 1000));
  const stateId = `wishResult_${Date.now().toString()}`;
  sessionStorage.setItem(stateId, JSON.stringify(wishResult));
  router.push({ path: "/wish-result", query: { stateId } });
}
/**
 * 重新开始许愿
 */
...
<template>
  <div class="home-container">
    ...
    <transition name="fade" mode="out-in">
      <div v-if="currentStep === 0" class="general-section" key="input">
